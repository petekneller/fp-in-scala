...
class Simulation {
  ...
  val stateOps = new StateOps[SimulationState]

  private def transitionState(f: MachineState => MachineState): State[SimulationState, Unit] =
    stateOps.modify(s => s.copy(
      machine = f(s.machine)
    ))

  def runCandyMachine(initialCandyMachine: MachineState, inputs: List[Input]): State[SimulationState, List[Unit]] = {

    val candyMachine = new CandyMachine

    val transitions: List[State[SimulationState, Unit]] = inputs.map{ input =>
      for {
  ...
  def summaryOfMachine(simulation: State[SimulationState, _]): State[SimulationState, (Int, Int)] = {
    for {
      _ <- simulation
