...
object CandyMachine {
  val stateM = new StateTOps[Simulation, Identity](new IdentityOps)

  def transitionMachine(input: Input)(f: Machine => Machine): StateT[Simulation, Identity, Unit] =
    stateM.modify(s => s.copy(
      machine = f(s.machine),
      recordedInputs = s.recordedInputs :+ input)
    )

  def summarizeMachine: StateT[Simulation, Identity, (Int, Int)] =
    stateM.get.map(s => (s.machine.candies, s.machine.coins))

  def simulateMachine(inputs: List[Input]): StateT[Simulation, Identity, (Int, Int)] = {

    def singleTransition(input: Input): StateT[Simulation, Identity, Unit] = {
      input match {
